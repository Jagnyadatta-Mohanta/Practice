const Storage = (() => {
  ...
  function setBooking(data) {
    ...
    sessionStorage.setItem(BOOKING_KEY, JSON.stringify({ ...current, ...data }));
  }
  ...
  function setShowtime(showtime) { setBooking({ showtime }); }
  ...
  function getPref(key, defaultVal = null) {
    return getPrefs()[key] ?? defaultVal;
  }
  ...
  function calculatePricing(movie, format, seatCount) {
    ...
    const priceMap = movie.price || {};
    const basePerSeat = priceMap[formatKey] || priceMap.standard || 400;
    const base = basePerSeat * seatCount;
    const convenience = Math.round(base * 0.04);
    const total = base + convenience;

    return { basePerSeat, base, convenience, total };
  }
...
const Auth = (() => {
  ...
  function updateUser(updates) {
    ...
    if (updates.newPassword) {
      ...
      if (user.passwordHash !== _hash(updates.currentPassword))
  ...
  return { signup, login, logout, getCurrentUser, getUser, updateUser, getInitials };
})();
