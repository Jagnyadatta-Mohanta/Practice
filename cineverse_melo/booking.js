...
function buildFilters(movie, lang) {
  ...
  if (cityInput) {
    ...
    if (user?.city && !cityInput.value) { cityInput.value = user.city; runFilter(); }
    cityInput.addEventListener('input', runFilter);
  }
  document.querySelector('#theater-input')?.addEventListener('input', runFilter);
}
...
function runFilter() {
  ...
  const filtered = theaters.filter(t =>
    (!cityQ    || t.city.toLowerCase().includes(cityQ)) &&
    (!theaterQ || t.name.toLowerCase().includes(theaterQ))
  );
...
function updateResultsBar(movie, lang, dateIdx) {
  ...
  if (!el) return;
  const d = new Date(); d.setDate(d.getDate() + dateIdx);
...
function renderTheaters(list, movie) {
  ...
  container.innerHTML = '';
  if (!list.length) {
    container.innerHTML = `<div class="theater-empty"><p>No cinemas found. Try a different city or theater name.</p></div>`;
    return;
  }
  list.forEach((t, i) => {
    ...
    const availText  = { available: 'Available', filling: 'Filling Fast', almost: 'Almost Full' }[availClass] || 'Available';
    card.innerHTML = `
      <div class="theater-card-header">
        <div class="theater-card-left">
          ...
          <div class="format-pills">${t.formats.map(f => `<span class="format-pill">${f}</span>`).join('')}</div>
        </div>
        <div class="theater-avail-badge ${availClass}">
          <span class="theater-avail-dot"></span>${availText}
        </div>
      </div>
      <div class="theater-card-body">
        <div class="timing-chips-row" role="group" aria-label="Showtimes for ${t.name}">
          ${t.timings.map(time => `<button class="timing-chip" data-time="${time}" aria-label="Select ${time}">${time}</button>`).join('')}
        </div>
    ...
    card.querySelectorAll('.timing-chip').forEach(chip => {
      chip.addEventListener('click', () => {
        ...
        Storage.setShowtime(chip.dataset.time);
      });
    });
    card.querySelector('.theater-book-btn').addEventListener('click', () => {
      if (!card.querySelector('.timing-chip.selected')) card.querySelector('.timing-chip')?.click();
