...
function buildHero() {
  ...
  heroMovies.forEach((m, i) => {
    ...
    dot.innerHTML = `
      <span class="hero-dot-label">${m.title.split(':')[0]}</span>
      <div class="hero-dot-line"></div>
    `;
...
function goToHero(idx, heroMovies) {
  ...
  setTimeout(() => {
    current?.classList.replace('leaving', 'entering');
...
function buildFilterBar() {
  ...
  // Genre / Language selects
  container.querySelector('#filter-genre').addEventListener('change', e => {
...
function applyFilters() {
  ...
  if (countEl) {
    const nowShowing = filteredMovies.filter(m => !m.comingSoon).length;
    countEl.textContent = `${nowShowing} film${nowShowing !== 1 ? 's' : ''}`;
  }
...
function buildSections(movies) {
  ...
  const sections = [
    {
      ...
      movies:   nowShowing.filter(m => m.category === 'Hollywood'),
    },
    {
      id:       'bollywood',
      title:    'Bollywood',
      subtitle: 'Desi Blockbusters',
      movies:   nowShowing.filter(m => m.category === 'Bollywood'),
    },
    {
      id:       'south',
      title:    'South',
      subtitle: 'Regional Powerhouses',
      movies:   nowShowing.filter(m => m.category === 'South'),
    },
    {
      id:       'trending',
      title:    'Trending Now',
      subtitle: 'Most Popular',
      movies:   nowShowing.filter(m => m.trending),
    },
    {
      id:       'latest',
      title:    'Latest Releases',
      subtitle: 'Just Arrived',
      movies:   [...nowShowing].sort((a, b) => new Date(b.releaseDate) - new Date(a.releaseDate)).slice(0, 8),
    },
    {
      id:       'coming-soon',
      title:    'Coming Soon',
      subtitle: 'Reserve Your Seats',
      movies:   coming,
      variant:  'coming-soon',
    },
  ];
  ...
  sections.forEach(cfg => {
    ...
    hasContent = true;
    const el = buildScrollSection(cfg);
    wrap.appendChild(el);
  });
  ...
  if (!hasContent) {
    wrap.innerHTML = `
      <div class="filter-empty" role="status">
        ...
        <button class="btn btn-secondary" onclick="document.querySelector('.filter-reset-btn')?.click()">
          Clear Filters
        </button>
      </div>
    `;
