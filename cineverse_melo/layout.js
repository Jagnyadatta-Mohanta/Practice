import { Auth, Storage } from './utils.js';
...
const Header = {
  ...
  _onSearchInput(input, dropdown) {
    ...
    dropdown.innerHTML = matches.map(m => `
      <a class="search-suggestion" href="movie.html?id=${m.id}" role="option"
        ...
        <div class="search-sug-poster"><img src="${m.poster}" alt="" loading="lazy"></div>
  ...
  _renderAuthArea() {
    ...
    area.querySelector('.hud-logout').addEventListener('click', () => Auth.logout());
    document.addEventListener('click', e => {
  ...
  _openAuthModal(defaultTab = 'login') {
    ...
    modal.addEventListener('keydown', e => {
      if (e.key === 'Enter') {
        modal.querySelector('.auth-panel:not([hidden])')?.querySelector('.auth-submit-btn')?.click();
      }
    });
  ...
  _bindHamburger() {
    ...
    btn.addEventListener('click', () => {
      ...
      if (!open) this._injectMobileThemeRow(nav);
    });
  },
  ...
  _renderThemeToggle() {
    const searchPlaceholder = document.querySelector('.header-search-placeholder');
    if (searchPlaceholder && !document.querySelector('.theme-toggle')) {
      const btn = document.createElement('button');
      btn.className = 'theme-toggle';
      btn.setAttribute('aria-label', 'Toggle theme');
      this._updateToggleIcon(btn);
      btn.addEventListener('click', () => this._toggleTheme(btn));
      searchPlaceholder.parentNode.insertBefore(btn, searchPlaceholder);
    }
  },
  ...
  _toggleTheme(btn) {
    ...
    if (pill) this._updatePillLabel(pill);
  },
  ...
  _updatePillLabel(pill) {
    const isDark = document.documentElement.getAttribute('data-theme') === 'dark';
    pill.innerHTML = isDark ? '☀️ Light Mode' : '🌙 Dark Mode';
  },

  _injectMobileThemeRow(nav) {
    if (nav.querySelector('.mobile-theme-row')) return;
    const isDark = document.documentElement.getAttribute('data-theme') === 'dark';
    const row = document.createElement('div');
    row.className = 'mobile-theme-row';
    row.innerHTML = `
      <span class="mobile-theme-label">Theme</span>
      <button class="theme-toggle-pill">${isDark ? '☀️ Light Mode' : '🌙 Dark Mode'}</button>`;
    row.querySelector('.theme-toggle-pill').addEventListener('click', () => {
      const desktopBtn = document.querySelector('.theme-toggle');
      this._toggleTheme(desktopBtn || document.createElement('button'));
      this._updatePillLabel(row.querySelector('.theme-toggle-pill'));
    });
    nav.appendChild(row);
  },
};
    ...
    <div class="footer-body">
        ...
        <div class="footer-brand">
          <a href="index.html" class="footer-logo" aria-label="Cineverse home">
            <div class="footer-logo-mark" aria-hidden="true">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="var(--void)" aria-hidden="true"><polygon points="5,3 19,12 5,21"/></svg>
            </div>
      ...
      <div class="footer-feedback">
        <div class="footer-feedback-inner">
          ...
          <textarea class="footer-feedback-textarea" placeholder="Share your thoughts..." aria-label="Feedback message" rows="3"></textarea>
...
function createMovieCard(movie) {
  ...
  card.innerHTML = `
    <div class="movie-card-poster">
      ...
      <div class="movie-card-rating" aria-label="Rating ${movie.rating} out of 10">★ ${movie.rating}</div>
...
function buildScrollSection(cfg, moviesArg) {
  let title, subtitle, movies, id, variant;
  if (cfg && typeof cfg === 'object' && !Array.isArray(cfg) && cfg.movies) {
    ({ title, movies, id = '', variant = '' } = cfg);
    subtitle = cfg.subtitle || '';
  } else {
    title = cfg; movies = moviesArg; subtitle = ''; id = ''; variant = '';
  }
  if (!movies?.length) return document.createDocumentFragment();
  ...
  movies.forEach(movie => track.appendChild(createMovieCard(movie)));
  ...
  leftBtn.addEventListener('click',  () => { track.scrollBy({ left: -400, behavior: 'smooth' }); setTimeout(updateButtons, 300); });
  rightBtn.addEventListener('click', () => { track.scrollBy({ left:  400, behavior: 'smooth' }); setTimeout(updateButtons, 300); });
